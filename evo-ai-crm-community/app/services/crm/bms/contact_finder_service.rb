class Crm::Bms::ContactFinderService
  ...
  def find_by_phone_number(contact)
    ...
    return if contact_data.blank? || contact_data.dig(:contact, :phone).nil?

    search_by_field(:phone, contact_data.dig(:contact, :phone))
  end
  ...
  def create_contact(contact)
    ...
    # BMS API doesn't return contact IDs, but successful creation is indicated by no error
    # Use placeholder ID or timestamp-based ID
    if contact_id.blank?
      contact_id = "bms-#{Time.current.to_i}-#{contact.id}"
      Rails.logger.info("BMS: Generated fallback contact ID: #{contact_id}")
    end
  ...
  def get_stored_id(contact)
    ...
    contact.additional_attributes.dig('external', 'bms_id')
  end
end
