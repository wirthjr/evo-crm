...
class Imap::BaseFetchEmailService
  ...
  def fetch_message_ids_with_sequence
    ...
    # Usar lotes maiores para alto volume
    batch_size = determine_batch_size(seq_nums.length)
    Rails.logger.info "[IMAP::FETCH_EMAIL_SERVICE] Using batch size: #{batch_size} for #{seq_nums.length} emails"

    message_ids_with_seq = []
    seq_nums.each_slice(batch_size).each do |batch|
  ...
  def determine_batch_size(total_emails)
    case total_emails
    when 0..50
      10  # Lotes pequenos para poucos emails
    when 51..200
      25  # Lotes médios
    when 201..1000
      50  # Lotes grandes para alto volume
    else
      100 # Lotes muito grandes para volume extremo
    end
  end
end
